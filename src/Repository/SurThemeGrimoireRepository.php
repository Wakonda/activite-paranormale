...
{
	protected function currentLanguages() {
		$queryLanguage = $this->getEntityManager()->createQueryBuilder();
		$queryLanguage->select("lcurrent.abbreviation")
					  ->from(Language::class, "lcurrent")
					  ->where("lcurrent.current = true");

		return $queryLanguage->getDQL();
	}
	...
	public function getAllThemesWorld()
	{
		...
		$qb ->join('c.language', 'l')
			...
			->setParameter('excludeLanguages', $this->currentLanguages())
			->orderBy("c.title");
